<script lang="ts">
  ...
  // Extract synchronized scroll engine context
  const lenisStore = getContext<Writable<Lenis | null>>('lenis');
  
  let time = 0;
  const mouse = { x: 0, y: 0 };
  const targetMouse = { x: 0, y: 0 };

  let heroMesh: THREE.Mesh | undefined = $state();
  let heroWireframe: THREE.Mesh | undefined = $state();
  let heroRing1: THREE.Mesh | undefined = $state();
  let heroRing2: THREE.Mesh | undefined = $state();
  let aboutKnot: THREE.Mesh | undefined = $state();
  let aboutWireframe: THREE.Mesh | undefined = $state();

  useTask((delta: number) => {
    if ($prefersReducedMotion || !$isCapableDevice) return;

    // STEP 1: Execute Lenis step update inside Threlte thread context.
    // This locks scroll paint updates perfectly to the WebGL rendering pass.
    if (lenisStore && $lenisStore) {
      $lenisStore.raf(window.performance.now());
    }

    // STEP 2: Progress timeline transformations via non-reactive references
    time += delta;
    ...
    if (type === 'hero') {
      if (heroMesh) {
        heroMesh.rotation.x = time * 0.1 + mouse.y * 0.3;
        heroMesh.rotation.y = time * 0.15 + mouse.x * 0.3;
      }
      if (heroWireframe) {
        heroWireframe.rotation.x = time * 0.05;
        heroWireframe.rotation.y = time * 0.08;
      }
      if (heroRing1) heroRing1.rotation.y = time * 0.3;
      if (heroRing2) heroRing2.rotation.y = time * -0.2;
    } else if (type === 'about') {
      if (aboutKnot) {
        aboutKnot.rotation.x = time * 0.1;
        aboutKnot.rotation.y = time * 0.2;
      }
      if (aboutWireframe) {
        aboutWireframe.rotation.x = time * 0.05;
        aboutWireframe.rotation.y = time * 0.1;
      }
    }
  });
...
{#if !$prefersReducedMotion && $isCapableDevice}
  ...
  {#if type === 'hero'}
    <Float speed={1.5} rotationIntensity={0.6} floatIntensity={0.8}>
      <T.Mesh bind:ref={heroMesh} position={[0, 0, 0]}>
  ...
  {:else if type === 'about'}
    <Float speed={1} rotationIntensity={0.4} floatIntensity={0.6}>
      <T.Mesh bind:ref={aboutKnot} position={[0, 0, -1]} rotation={[0, 0, Math.PI / 4]}>
    ...
    <T.Mesh bind:ref={aboutWireframe} position={[0, 0, -1]} rotation={[0, 0, Math.PI / 4]}>
  ...
  {/if}
{:else}
  <div class="fixed inset-0 z-[-1] bg-slate-950 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950"></div>
{/if}